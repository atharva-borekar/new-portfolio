...
import {
  MenuToggle,
  NavigationList,
} from "components/Navigation/NavComponents";
...
export default function Navigation() {
  ...
  return (
    <motion.div
      ...
      initial={{ width: 80 }}
      animate={{ width: isOpen ? 300 : 80 }} // Animate the width
      transition={{ type: "spring", stiffness: 300, damping: 30 }} // Smooth transition for width
      style={{ overflow: "hidden" }} // Ensure overflow is hidden when closed
    >
      <motion.nav
        initial={false}
...
const sidebarVariants = {
  open: (height = 1000) => ({
    ...
    width: "300px", // Ensure the width is set when open
  }),
  closed: {
    clipPath: "circle(30px at 40px 40px)",
    zIndex: 0, // Keep it beneath the toggle
    transition: {
      delay: 0.2,
      type: "spring",
      stiffness: 400,
